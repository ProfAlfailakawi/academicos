        ...
        <Card className="xl:sticky xl:top-24">
          <CardContent className="p-3">
            <div className="px-2 py-2 flex items-center justify-between gap-3">
              ...
              <span className="rounded-full brand-soft-bg px-2 py-1 text-[11px] font-semibold">{document.variation.id}</span>
            </div>
            ...
            <CardContent className="p-0">
              <div className="px-4 md:px-5 py-4 border-b hairline flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[11px] muted">{draft.trim() ? draft.trim().split(/\s+/u).length : 0} {t("writer.words")}</span>
              ...
              <textarea
                ...
                spellCheck
              />
              <div className="px-5 py-3 border-t hairline text-[11px] muted flex flex-wrap justify-between gap-2">
...
function QualityCard({ document }: { document: ProjectDocument }) {
  ...
  return <Card><CardContent><div className="flex items-center justify-between gap-3"><div><div className="eyebrow">{t("writer.health")}</div><h3 className="text-sm font-semibold mt-1">{t("writer.quickPanel")}</h3></div><PanelRightOpen size={17} className="muted" /></div><div className="quality-rings grid grid-cols-2 gap-3 mt-5">{metrics.map(([label, value]) => <div key={label} className="quality-ring" style={{ "--score": value } as React.CSSProperties}><div className="quality-ring__dial"><strong>{value}</strong><span>%</span></div><div className="text-[11px] font-semibold mt-2">{label}</div></div>)}</div>{document.integrityWarnings[0] && <div className="rounded-xl bg-warning/10 text-warning p-3 text-[11px] leading-5 mt-4 flex gap-2"><AlertTriangle size={13} className="shrink-0 mt-0.5" />{document.integrityWarnings[0]}</div>}</CardContent></Card>;
}
