...
@media print{body{background:#0b1310}.notes{display:none}.slide{height:100vh;min-height:0}}
  ...
  return <div className="space-y-6">
    <div className="rounded-2xl border hairline bg-gradient-to-r from-warning/10 via-warning/8 to-transparent p-5 md:p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
      <div className="flex items-center gap-3.5"><div className="h-11 w-11 rounded-2xl bg-warning/20 text-warning grid place-items-center shrink-0"><Presentation size={22}/></div><div><div className="flex items-center gap-2 flex-wrap"><span className="text-[11px] font-bold tracking-wider uppercase text-warning">{t("ui.evidenceBoundPresentation")}</span><span className="px-2 py-0.5 rounded-full text-[11px] bg-warning/15 text-warning font-semibold border border-warning/20">{t("ui.projectDnaOnly")}</span></div><h2 className="text-lg md:text-xl font-bold tracking-tight mt-0.5">{t("pres.title")}</h2><p className="text-xs text-muted-foreground mt-1">{t("pres.description")}</p></div></div>
    ...
    <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-6 items-start">
      <div className="space-y-4"><div className="surface-deep slide-canvas rounded-3xl p-8 md:p-10 shadow-2xl min-h-[360px] flex flex-col justify-between">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-line text-[11px] text-muted-foreground font-mono"><span>{project.course}</span><span className="flex items-center gap-1"><ShieldCheck size={11}/>{t("pres.bound")}</span></div>
      </div>
