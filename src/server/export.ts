...
function exportLocale(project:ProjectDNA,branding:ExportBranding={}):ExportLocale{const raw=String(branding.locale||project.language||'en').trim().toLowerCase();const short=raw.slice(0,2) as ExportLocale;return short in EXPORT_LOCALES?short:'en';}
...
function markdown(project: ProjectDNA, artifacts: WorkspaceArtifact[] = [], branding:ExportBranding={}) {
  ...
  const deadline=project.deadlines.final||w('needsConfirmation',locale);
  const lines = [
    ...(branding.institutionName?[`**${w('institution',locale)}:** ${branding.institutionName}`,'']:[]),`# ${project.title}`,'',`- **${w('course',locale)}:** ${project.course}`,`- **${w('projectType',locale)}:** ${project.projectType}`,`- **${w('status',locale)}:** ${project.status}`,`- **${w('progress',locale)}:** ${project.progress}%`,
    ...
    ...(project.deliverables.length?project.deliverables.map(d => `- [${d.status === 'completed' || d.status === 'ready' ? 'x' : ' '}] ${d.title} — ${d.format}`):[`- ${w('noDeliverables',locale)}`]),'',`## ${w('requirements',locale)}`,
    ...(project.requirements.length?project.requirements.map(r => `- **${r.label}:** ${r.value} (${r.confidence})`):[`- ${w('needsConfirmation',locale)}`]),'',`## ${w('tasks',locale)}`,...project.tasks.map(t => `- [${t.status === 'completed' ? 'x' : ' '}] ${t.title} — ${t.status}`),'',`## ${w('rubric',locale)}`,
    ...
    ...(artifacts.length ? artifacts.map(a => `- **${a.title}** — ${a.module} / ${a.status}${a.deliverableId ? ` → ${w('deliverable',locale)} ${a.deliverableId}` : ''}${a.rubricIds?.length ? ` → ${w('rubric',locale)} ${a.rubricIds.join(', ')}` : ''}${a.isCanonical ? ' — canonical' : ''}`) : [`- ${w('noArtifacts',locale)}`]),'','---',
    `${w('exportedAt',locale)}: ${new Date().toISOString()}. ${w('exportFooter',locale)}${branding.footer?` ${branding.footer}`:''}`,
  ]; return lines.join('\n');
}
...
function projectRows(project:ProjectDNA,branding:ExportBranding={}){
  ...
  return [
    [w('field',locale),w('value',locale)],...(branding.institutionName?[[w('institution',locale),branding.institutionName]]:[]),[w('title',locale),project.title],[w('course',locale),project.course],[w('projectType',locale),project.projectType],[w('domain',locale),project.academicDomain],[w('status',locale),project.status],[w('progress',locale),`${project.progress}%`],[w('deadline',locale),project.deadlines.final||w('needsConfirmation',locale)],[w('aiPolicy',locale),`${w('level',locale)} ${project.aiPolicy.level} — ${project.aiPolicy.summary}`],
    ...project.deliverables.map(d=>[`${w('deliverable',locale)}: ${d.title}`,`${d.format} — ${d.status}`]),...project.tasks.map(t=>[`${w('task',locale)}: ${t.title}`,t.status]),...project.rubric.map(r=>[`${w('rubric',locale)}: ${r.title}`,`${r.weighting}% — ${r.readiness||'not evidenced'}`]),
  ];
}
...
function docx(project:ProjectDNA,artifacts:WorkspaceArtifact[],branding:ExportBranding={}){
  const locale=exportLocale(project,branding), meta=EXPORT_LOCALES[locale], rtl=meta.dir==='rtl';
  const sections=academicSections(artifacts);
  const paras=[...(branding.institutionName?[branding.institutionName]:[]),project.title,`${w('course',locale)}: ${project.course}`,`${w('projectType',locale)}: ${project.projectType}`,`${w('deadline',locale)}: ${project.deadlines.final||w('needsConfirmation',locale)}`,...sections.flatMap(section=>[section.title,...section.content.split(/\n+/).filter(Boolean)]),w('rubric',locale),...(project.rubric.length?project.rubric.map(r=>`${r.title} (${r.weighting}%): ${r.description||''} — ${r.readiness||'not evidenced'}`):[w('noRubric',locale)]),w('aiDisclosure',locale),project.aiPolicy.summary,w('exportFooter',locale)];
...
function pptx(project:ProjectDNA,branding:ExportBranding={}){
  ...
  const slides=[
    ...
    [w('deliverables',locale),project.deliverables.map(d=>`${d.title} — ${d.status}`).join('\n')||w('noDeliverables',locale)],
    [w('rubricReadiness',locale),project.rubric.map(r=>`${r.title}: ${r.readiness||'not evidenced'}`).join('\n')||w('noRubricShort',locale)],
