...
export function SearchWorkspace(){
  ...
  return <div className="space-y-6"><PageHeader eyebrow={t("ui.globalSearch")} title={t('search.title')} description={t('search.description')}/>
    ...
    <Card><CardContent><div className="flex items-center justify-between"><div><div className="eyebrow">{t("ui.permissionAwareResults")}</div><h2 className="section-title mt-1">{t('search.results')}</h2></div>{loading?<LoaderCircle size={18} className="animate-spin brand-text"/>:<span className="text-[11px] muted mono-number">{results.length}</span>}</div>
