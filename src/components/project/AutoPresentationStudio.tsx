...
@media print{body{background:#0b1310}.notes{display:none}.slide{height:100vh;min-height:0}}
  ...
  return <div className="space-y-6">
    <div className="rounded-2xl border hairline bg-gradient-to-r from-warning/10 via-warning/8 to-transparent p-5 md:p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
      <div className="flex items-center gap-3.5"><div className="h-11 w-11 rounded-2xl bg-warning/20 text-warning grid place-items-center shrink-0"><Presentation size={22}/></div><div><div className="flex items-center gap-2 flex-wrap"><span className="text-[10px] font-bold tracking-wider uppercase text-warning">{t("ui.evidenceBoundPresentation")}</span><span className="px-2 py-0.5 rounded-full text-[10px] bg-warning/15 text-warning font-semibold border border-warning/20">{t("ui.projectDnaOnly")}</span></div><h2 className="text-lg md:text-xl font-bold tracking-tight mt-0.5">{t("pres.title")}</h2><p className="text-xs text-muted-foreground mt-1">{t("pres.description")}</p></div></div>
    ...
    <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-6 items-start">
      <div className="space-y-4"><div className="surface-deep slide-canvas rounded-3xl p-8 md:p-10 shadow-2xl min-h-[360px] flex flex-col justify-between">
        <div><div className="flex items-center justify-between text-[11px] text-muted-foreground pb-4 border-b border-line"><span className="font-bold tracking-wider uppercase text-warning">{project.academicDomain || t("pres.project")}</span><span className="font-mono">{currentSlide.id} / {slides.length}</span></div><h2 className="text-xl md:text-2xl font-bold mt-5">{currentSlide.title}</h2><p className="text-xs text-warning mt-1">{currentSlide.subtitle}</p></div>
        <div className="space-y-3 py-6">{currentSlide.bulletPoints.map((point,i) => <div key={i} className="flex items-start gap-3 text-xs md:text-sm text-ink"><span className="h-2 w-2 rounded-full bg-warning mt-2 shrink-0"/><span className="leading-relaxed">{point}</span></div>)}</div>
        <div className="flex items-center justify-between pt-4 border-t border-line text-[10px] text-muted-foreground font-mono"><span>{project.course}</span><span className="flex items-center gap-1"><ShieldCheck size={11}/>{t("pres.bound")}</span></div>
      </div>
      <div className="flex items-center justify-between p-3 rounded-2xl border hairline bg-[var(--panel)]"><Button size="sm" variant="outline" onClick={() => setCurrentSlideIndex(p => Math.max(0,p-1))} disabled={currentSlideIndex===0}><ChevronLeft size={14} className="directional-icon"/>{t("pres.previous")}</Button><div className="flex gap-1.5">{slides.map((s,idx)=><button key={s.id} onClick={()=>setCurrentSlideIndex(idx)} className={`h-2.5 rounded-full ${currentSlideIndex===idx?"w-7 bg-warning":"w-2.5 bg-muted"}`}/>)}</div><Button size="sm" variant="outline" onClick={() => setCurrentSlideIndex(p => Math.min(slides.length-1,p+1))} disabled={currentSlideIndex===slides.length-1}>{t("pres.next")}<ChevronRight size={14} className="directional-icon"/></Button></div></div>

      <Card className="rounded-3xl border hairline bg-[var(--panel)]"><CardContent className="p-6 md:p-7 space-y-5"><div className="flex items-center justify-between border-b hairline pb-4"><div className="flex items-center gap-2 text-warning text-xs font-bold uppercase tracking-wider"><Mic size={15}/>{t("ui.presenterCoach")}</div><span className="flex items-center gap-1 text-[11px] font-mono font-bold bg-warning/10 text-warning px-2.5 py-0.5 rounded-full"><Clock size={12}/>{currentSlide.allocatedTimeSeconds} {t("pres.seconds")}</span></div>
        <div className="p-4 rounded-2xl bg-warning/10 border border-warning/10"><div className="text-[11px] font-semibold text-warning">{t("pres.safeScript")}</div><p className="text-xs md:text-sm leading-relaxed font-medium mt-2">{currentSlide.speakerScript}</p><Button size="sm" variant="ghost" className="mt-3" onClick={copyScript}>{copied?<Check size={13} className="text-success"/>:<Copy size={13}/>} {copied?t("pres.copied"):t("pres.copy")}</Button></div>
        <div className="p-3.5 rounded-xl border hairline bg-[var(--bg)] text-xs text-muted-foreground"><span className="font-bold text-foreground flex items-center gap-1.5"><Sparkles size={13} className="text-warning"/>{t("pres.slideGoal")}</span><p className="mt-1 leading-relaxed">{currentSlide.keyTakeaway}</p></div>
