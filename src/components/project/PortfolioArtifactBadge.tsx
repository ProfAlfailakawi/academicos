...
export function PortfolioArtifactBadge({ project }: { project: ProjectDNA }) {
  ...
  async function createShare() {
    ...
    try { const r = await api.createShare({ kind: "project", targetId: project.id, label: `Portfolio proof · ${project.title}`, watermark: "AcademicOS Portfolio Proof" }); const url = new URL(r.url, window.location.origin).toString(); setShareUrl(url); await navigator.clipboard.writeText(url); setCopied(true); window.setTimeout(() => setCopied(false), 1800); }
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border hairline bg-gradient-to-r from-cyan-500/10 via-sky-500/5 to-transparent p-5 md:p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5"><div className="h-11 w-11 rounded-2xl bg-cyan-500/20 text-cyan-600 grid place-items-center shrink-0"><Award size={22}/></div><div><div className="text-[10px] font-bold tracking-wider uppercase text-cyan-600">Portfolio Proof</div><h2 className="text-lg md:text-xl font-bold tracking-tight mt-0.5">{t("portfolio.title")}</h2><p className="text-[11px] text-muted-foreground mt-1">{t("portfolio.description")}</p></div></div>
      ...
      {loading ? <div className="min-h-40 grid place-items-center"><LoaderCircle className="animate-spin text-cyan-600"/></div> : (
        <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-6 items-start">
          <div className="rounded-3xl border-2 hairline bg-gradient-to-b from-[var(--panel)] to-[var(--bg)] p-6 md:p-8 space-y-6 shadow-xl">
            <div className="flex items-start justify-between gap-3"><div><span className="text-[10px] font-mono uppercase text-muted-foreground">Project evidence artifact</span><h3 className="text-base font-bold mt-1">{project.title}</h3><p className="text-[11px] text-muted-foreground mt-1">{project.course} · {project.status}</p></div><span className="px-2.5 py-1 rounded-full text-[10px] font-semibold bg-cyan-500/10 text-cyan-600 border border-cyan-500/20">{project.progress}% project progress</span></div>
            <div className="grid grid-cols-3 gap-2 text-center"><Metric label={t("portfolio.completedDeliverables")} value={completedDeliverables}/><Metric label="Evidence" value={capsule?.provenance.evidenceItems || 0}/><Metric label="Proof of Learning" value={capsule?.proofOfLearning.length || 0}/></div>
