...
export const BILLING_PLANS = [
  { id: 'preview', name: 'Free preview', amountUsd: 0, pages: 3, projects: 1, description: 'Assignment analysis and a guided preview before payment' },
  { id: 'project', name: 'Full project', amountUsd: 6.99, pages: 20, projects: 1, description: 'Full project workspace, X-Ray, export, source checks and revisions' },
  { id: 'project_viva', name: 'Project + Viva', amountUsd: 8.99, pages: 25, projects: 1, description: 'Everything in Full Project plus personalized viva preparation' },
  { id: 'group', name: 'Team project', amountUsd: 12.99, pages: 35, projects: 1, description: 'Shared project workspace with roles, evidence and one final delivery' },
] as const;
...
const localBillingCurrency = () => String(process.env.BILLING_LOCAL_CURRENCY || 'USD').trim().toUpperCase();
const localUnitsPerUsd = () => Math.max(0.000001, Number(process.env.BILLING_LOCAL_UNITS_PER_USD || 1));
const localAmount = (usd:number) => Number((usd * localUnitsPerUsd()).toFixed(localBillingCurrency() === 'KWD' ? 3 : 2));
...
class StripeBillingProvider implements BillingProvider {
  ...
  async createCheckout(input: BillingCheckoutInput) {
    ...
    body.set('line_items[0][price_data][currency]', 'usd'); body.set('line_items[0][price_data][unit_amount]', String(Math.round(input.amountUsd * 100))); body.set('line_items[0][price_data][product_data][name]', input.description); body.set('line_items[0][quantity]', '1');
...
class TapBillingProvider implements BillingProvider {
  ...
  async createCheckout(input:BillingCheckoutInput){
    ...
    const json=await providerJson('https://api.tap.company/v2/charges/',{method:'POST',headers:{Authorization:`Bearer ${process.env.TAP_SECRET_KEY}`,'Content-Type':'application/json','Idempotency-Key':input.idempotencyKey},body:JSON.stringify({amount:localAmount(input.amountUsd),currency:localBillingCurrency(),customer_initiated:true,threeDSecure:true,save_card:false,description:input.description,metadata:{tenantId:input.tenantId,userId:input.userId,projectId:input.projectId,planId:input.planId,idempotencyKey:input.idempotencyKey},reference:{transaction:input.idempotencyKey,order:input.idempotencyKey},receipt:{email:Boolean(input.customerEmail),sms:false},customer:{first_name:(input.customerName||'AcademicOS').slice(0,80),email:input.customerEmail},merchant:{id:process.env.TAP_MERCHANT_ID},source:{id:process.env.TAP_SOURCE_ID||'src_all'},post:{url:input.webhookUrl},redirect:{url:input.successUrl}})});
...
class MyFatoorahBillingProvider implements BillingProvider {
  ...
  async createCheckout(input:BillingCheckoutInput){
    ...
    const json=await providerJson(`${base}/v2/ExecutePayment`,{method:'POST',headers:{Authorization:`Bearer ${process.env.MYFATOORAH_API_TOKEN}`,'Content-Type':'application/json'},body:JSON.stringify({InvoiceValue:localAmount(input.amountUsd),PaymentMethodId:Number(process.env.MYFATOORAH_PAYMENT_METHOD_ID),CustomerName:(input.customerName||'AcademicOS user').slice(0,100),CustomerEmail:input.customerEmail,DisplayCurrencyIso:localBillingCurrency(),CallBackUrl:input.successUrl,ErrorUrl:input.cancelUrl,Language:(process.env.MYFATOORAH_LANGUAGE||'EN').toUpperCase(),CustomerReference:input.projectId,UserDefinedField:JSON.stringify({tenantId:input.tenantId,userId:input.userId,projectId:input.projectId,planId:input.planId,idempotencyKey:input.idempotencyKey}).slice(0,500)})});
