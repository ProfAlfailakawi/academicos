    ...
    <div className="space-y-6">
      ...
      {source ? (
        <div className="rounded-2xl border hairline bg-[var(--panel)] p-5 space-y-5">
          <div className="flex items-start justify-between gap-3"><div><div className="inline-flex items-center gap-1 text-[11px] text-success font-semibold"><Check size={12}/>{t("ui.crossrefMetadataMatched")}</div><h3 dir="auto" className="text-sm font-bold mt-1">{source.title}</h3><p dir="auto" className="citation text-[11px] text-muted-foreground mt-1">{authorsPlain(source)}{source.year ? ` · ${source.year}` : ""}{source.containerTitle ? ` · ${source.containerTitle}` : ""}</p></div><span className="font-mono text-[11px] text-muted-foreground ltr break-all max-w-[35%]">{source.doi}</span></div>
          <div className="grid md:grid-cols-2 gap-3"><div className="rounded-xl border hairline bg-[var(--bg)] p-4"><div className="flex items-center gap-2 text-[11px] font-semibold"><BookMarked size={14} className="text-insight"/>{t("formatter.inText")} · {STYLE_NAME[selectedStyle]}</div><div dir="auto" className="citation mt-3 text-sm font-medium">{inText(source, selectedStyle)}</div></div><div className="rounded-xl border hairline bg-[var(--bg)] p-4"><div className="text-[11px] font-semibold">{t("formatter.references")} · {STYLE_NAME[selectedStyle]}</div><p dir="auto" className="citation mt-3 text-xs leading-6">{formatted(source, selectedStyle)}</p></div></div>
