...
import { BookOpenCheck, FileText, FolderKanban, Search } from "lucide-react";
...
export function SearchWorkspace(){
  ...
  return <div className="space-y-6"><PageHeader eyebrow={t("ui.globalSearch")} title={t('search.title')} description={t('search.description')}/>
    ...
    <Card><CardContent><div className="flex items-center justify-between"><div><div className="eyebrow">{t("ui.permissionAwareResults")}</div><h2 className="section-title mt-1">{t('search.results')}</h2></div>{loading?<InlineLoader size={18}/>:<span className="text-[11px] muted mono-number">{results.length}</span>}</div>
...
function Result({item}:{item:GlobalSearchItem}){const{t,locale}=useI18n();const Icon=item.type==='project'?FolderKanban:item.type==='course'?BookOpenCheck:FileText;const label=item.type==='project'?t('search.typeProject'):item.type==='course'?t('search.typeCourse'):t('search.typeAssignment');return <Link to={item.path} className="focus-ring flex items-center gap-4 py-4 px-2 rounded-xl hover:bg-[var(--panel-2)]"><div className="h-10 w-10 rounded-xl tone-tile shrink-0"><Icon size={17}/></div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h3 className="text-sm font-semibold truncate">{item.title}</h3><span className="rounded-full soft-bg px-2 py-0.5 text-[11px] muted shrink-0">{label}</span></div><p className="text-[11px] muted mt-1 truncate">{item.subtitle}</p></div><time className="hidden sm:block text-[11px] muted shrink-0">{formatDate(item.updatedAt,locale)}</time></Link>}
