...
const amountKwd = () => {
  const amount = Number(process.env.BILLING_STUDENT_PRO_AMOUNT_KWD || 0);
  if (!Number.isFinite(amount) || amount <= 0) throw Object.assign(new Error('The Student Pro KWD amount is not configured'), { code: 'BILLING_NOT_CONFIGURED' });
...
class StripeBillingProvider implements BillingProvider {
  id = 'stripe' as const;
  configured() { return Boolean(process.env.STRIPE_SECRET_KEY && process.env.STRIPE_WEBHOOK_SECRET && process.env.STRIPE_STUDENT_PRO_PRICE_ID); }
  async createCheckout(input: BillingCheckoutInput) {
    if (!this.configured()) throw Object.assign(new Error('Stripe credentials, webhook secret, or price ID are not configured'), { code: 'BILLING_NOT_CONFIGURED' });
    const body = new URLSearchParams();
    body.set('mode', 'subscription'); body.set('success_url', input.successUrl); body.set('cancel_url', input.cancelUrl);
    body.set('line_items[0][price]', process.env.STRIPE_STUDENT_PRO_PRICE_ID!); body.set('line_items[0][quantity]', '1');
    if (input.customerEmail) body.set('customer_email', input.customerEmail);
    body.set('metadata[tenantId]', input.tenantId); body.set('metadata[userId]', input.userId);
    body.set('subscription_data[metadata][tenantId]', input.tenantId); body.set('subscription_data[metadata][userId]', input.userId);
...
class MyFatoorahBillingProvider implements BillingProvider {
  ...
  configured(){return Boolean(process.env.MYFATOORAH_API_TOKEN&&process.env.MYFATOORAH_PAYMENT_METHOD_ID&&process.env.MYFATOORAH_WEBHOOK_SECRET&&Number(process.env.BILLING_STUDENT_PRO_AMOUNT_KWD)>0);}
  async createCheckout(input:BillingCheckoutInput){
    if(!this.configured())throw Object.assign(new Error('MyFatoorah token, payment method, webhook secret, or amount are not configured'),{code:'BILLING_NOT_CONFIGURED'});
    ...
    const json=await providerJson(`${base}/v2/ExecutePayment`,{method:'POST',headers:{Authorization:`Bearer ${process.env.MYFATOORAH_API_TOKEN}`,'Content-Type':'application/json'},body:JSON.stringify({InvoiceValue:amountKwd(),PaymentMethodId:Number(process.env.MYFATOORAH_PAYMENT_METHOD_ID),CustomerName:(input.customerName||'AcademicOS user').slice(0,100),CustomerEmail:input.customerEmail,DisplayCurrencyIso:'KWD',CallBackUrl:input.successUrl,ErrorUrl:input.cancelUrl,Language:'AR',UserDefinedField:JSON.stringify({tenantId:input.tenantId,userId:input.userId,idempotencyKey:input.idempotencyKey}).slice(0,500)})});
...
export function billingStatus() {
  ...
  return { provider: active.id, configured: active.configured(), readiness };
}

export interface VerifiedPaymentEvent {provider:'tap'|'myfatoorah';eventId:string;tenantId:string;userId:string;externalId:string;status:'paid'|'failed'|'pending'|'refunded'|'chargeback';amount:number;currency:string;rawType:string}
...
export function verifyTapWebhook(raw:Buffer,signature:string):VerifiedPaymentEvent{
  ...
  const state=String(body.status||'').toUpperCase();const status:VerifiedPaymentEvent['status']=state==='CAPTURED'?'paid':state==='REFUNDED'?'refunded':state.includes('FAILED')||state==='DECLINED'||state==='CANCELLED'?'failed':'pending';
  return{provider:'tap',eventId:String(body.id||''),tenantId:String(body.metadata?.tenantId||''),userId:String(body.metadata?.userId||''),externalId:String(body.id||''),status,amount,currency,rawType:state};
}
