...
export function MissionControl() {
  ...
  return <div className="student-home space-y-6 md:space-y-8">
    ...
    <Card className="understanding-card overflow-hidden"><CardContent><div className="flex items-center gap-3"><span className="h-11 w-11 rounded-2xl tone-tile grid place-items-center"><BrainCircuit size={19}/></span><div><div className="eyebrow">{t("ui.learningBrain")}</div><h2 className="section-title mt-1">{t("mission.brainTitle")}</h2></div></div>{brain?.recommendedFocus?.[0]?<div className="mt-5 rounded-2xl bg-white/55 p-4"><div className="text-[10px] opacity-70">{t("mission.nextFocus")}</div><strong className="block mt-1">{brain.recommendedFocus[0].skill}</strong><p className="text-xs leading-6 mt-2 opacity-80">{t("mission.nextFocusDesc")}</p></div>:<p className="text-xs leading-6 mt-5 text-[var(--brand-2)]">{t("mission.brainEmpty")}</p>}<div className="grid grid-cols-3 gap-2 mt-4"><BrainStat label={t("mission.projects")} value={brain?.projectsAnalyzed||0}/><BrainStat label={t("mission.learningEvidence")} value={brain?.evidenceItems||0}/><BrainStat label={t("mission.strengths")} value={brain?.strengths.length||0}/></div></CardContent></Card></section>}
    ...
    <section aria-labelledby="academic-toolkit-title" className="rounded-[28px] border hairline bg-[var(--panel)] p-5 md:p-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-3">
        ...
        <button type="button" onClick={()=>window.dispatchEvent(new Event("academicos:open-palette"))} className="hidden md:inline-flex focus-ring items-center gap-2 rounded-xl border hairline px-3 py-2 text-[10px] font-semibold muted"><Sparkles size={13}/>{t("layout.searchAndNavigate")}</button>
      </div>
