  ...
  return <div className="student-home space-y-6 md:space-y-8">
    <section className="student-hero panel-flat rounded-[32px] overflow-hidden"><div className="grid lg:grid-cols-[1.03fr_.97fr] items-stretch">
      <div className="p-6 md:p-9 lg:p-11">
        ...
        {error&&<div role="alert" className="mt-3 rounded-xl bg-red-500/15 border border-red-500/30 text-red-950 dark:text-red-200 p-3.5 text-xs font-medium leading-relaxed">{error}</div>}
      </div>
    ...
    <Card className="understanding-card overflow-hidden"><CardContent><div className="flex items-center gap-3"><span className="h-11 w-11 rounded-2xl bg-white/70 text-[var(--brand-2)] grid place-items-center"><BrainCircuit size={19}/></span><div><div className="eyebrow">{t("ui.learningBrain")}</div><h2 className="section-title mt-1">{t("mission.brainTitle")}</h2></div></div>{brain?.recommendedFocus?.[0]?<div className="mt-5 rounded-2xl bg-white/55 p-4"><div className="text-[10px] opacity-70">{t("mission.nextFocus")}</div><strong className="block mt-1">{brain.recommendedFocus[0].skill}</strong><p className="text-xs leading-6 mt-2 opacity-80">{t("mission.nextFocusDesc")}</p></div>:<p className="text-xs leading-6 mt-5 text-[var(--brand-2)]">{t("mission.brainEmpty")}</p>}<div className="grid grid-cols-3 gap-2 mt-4"><BrainStat label={t("mission.projects")} value={brain?.projectsAnalyzed||0}/><BrainStat label={t("mission.learningEvidence")} value={brain?.evidenceItems||0}/><BrainStat label={t("mission.strengths")} value={brain?.strengths.length||0}/></div></CardContent></Card></section>}
