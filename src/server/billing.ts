...
export const BILLING_PLANS = [
  { id: 'preview', name: 'معاينة مجانية', amountKwd: 0, pages: 2, projects: 1, description: 'تحليل التكليف وخطة أولية' },
...
class StripeBillingProvider implements BillingProvider {
  ...
  async createCheckout(input: BillingCheckoutInput) {
    ...
    body.set('metadata[tenantId]', input.tenantId); body.set('metadata[userId]', input.userId); body.set('metadata[planId]', input.planId);
...
export interface VerifiedPaymentEvent {provider:'tap'|'myfatoorah';eventId:string;tenantId:string;userId:string;planId?:string;externalId:string;status:'paid'|'failed'|'pending'|'refunded'|'chargeback';amount:number;currency:string;rawType:string}
...
export function verifyTapWebhook(raw:Buffer,signature:string):VerifiedPaymentEvent{
  ...
  return{provider:'tap',eventId:String(body.id||''),tenantId:String(body.metadata?.tenantId||''),userId:String(body.metadata?.userId||''),planId:String(body.metadata?.planId||'')||undefined,externalId:String(body.id||''),status,amount,currency,rawType:state};
}
