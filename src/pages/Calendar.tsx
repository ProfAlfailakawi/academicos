...
export function Calendar() {
  ...
  return <div className="space-y-7"><PageHeader eyebrow="Semester OS" title={t('calendar.title')} description={t('calendar.description')}/>{projects===null?<div className="h-80 soft-bg rounded-2xl animate-pulse"/>:events.length?<Card><CardContent className="p-2 md:p-3">{events.map((e,i)=><Link to={`/app/project/${e.projectId}`} key={e.id} className="focus-ring flex items-center gap-4 rounded-xl p-3 md:p-4 hover:bg-[var(--panel-2)]"><div className="h-12 w-12 rounded-xl brand-soft-bg flex flex-col items-center justify-center shrink-0"><span className="text-[10px] uppercase">{new Date(e.date).toLocaleDateString(document.documentElement.lang || undefined,{month:'short'})}</span><span className="font-semibold mono-number">{new Date(e.date).getDate()}</span></div><div className="min-w-0 flex-1"><div className="text-sm font-semibold truncate">{e.title}</div><div className="text-[11px] muted mt-1 truncate">{e.subtitle}</div></div><div className="hidden sm:flex items-center gap-1 text-[11px] muted"><Clock3 size={13}/>{new Date(e.date).toLocaleTimeString(document.documentElement.lang || undefined,{hour:'2-digit',minute:'2-digit'})}</div></Link>)}</CardContent></Card>:<EmptyState title={t('calendar.emptyTitle')} description={t('calendar.emptyDescription')}/>}</div>;
}
