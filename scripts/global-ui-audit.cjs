...
expect('Bootstrap supports 8 locales with Arabic-first fallback', LOCALES.every(c=>bootstrap.includes(`"${c}"`)) && bootstrap.includes('code = code || "ar"') && /<html lang="ar" dir="rtl">/.test(html), 'bootstrap locale list/fallback incomplete or index.html default is not ar/rtl');
