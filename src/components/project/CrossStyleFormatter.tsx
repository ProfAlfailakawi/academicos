    ...
    <div className="space-y-6">
      ...
      {source ? (
        <div className="rounded-2xl border hairline bg-[var(--panel)] p-5 space-y-5">
          <div className="flex items-start justify-between gap-3"><div><div className="inline-flex items-center gap-1 text-[11px] text-success font-semibold"><Check size={12}/>{t("ui.crossrefMetadataMatched")}</div><h3 className="text-sm font-bold mt-1">{source.title}</h3><p className="text-[11px] text-muted-foreground mt-1">{authorsPlain(source)}{source.year ? ` · ${source.year}` : ""}{source.containerTitle ? ` · ${source.containerTitle}` : ""}</p></div><span className="font-mono text-[11px] text-muted-foreground ltr break-all max-w-[35%]">{source.doi}</span></div>
