...
const routeModules=fs.existsSync(path.join(root,'src/server/routes'))?fs.readdirSync(path.join(root,'src/server/routes')).filter(f=>f.endsWith('.ts')).sort().map(f=>read(path.join('src/server/routes',f))):[];
