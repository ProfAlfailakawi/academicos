...
import { AlertTriangle, ArrowRight, CalendarClock, CheckCircle2, Clock3, FolderKanban, Plus, Sparkles } from 'lucide-react';
    ...
    <div className="space-y-7 md:space-y-9">
      ...
      <div className="grid xl:grid-cols-[1.5fr_.8fr] gap-5">
        <section className="space-y-4 min-w-0">
          <div className="flex items-center justify-between"><h2 className="section-title">{t('dash.currentProjects')}</h2><Button variant="ghost" size="sm" asChild><Link to="/app/projects">{t('dash.viewAll')} <ArrowRight size={14} className="directional-icon"/></Link></Button></div>
...
function NextAction({ project }: { project: ProjectDNA }) {
  const { t } = useI18n();
  return <section className="relative overflow-hidden rounded-[24px] brand-hero p-6 md:p-8 shadow-xl"><div className="absolute inset-0 opacity-[.12] paper-grid"/><div className="relative grid md:grid-cols-[1fr_auto] gap-6 items-end"><div><div className="flex items-center gap-2 text-white/70 text-xs font-semibold mb-3"><Sparkles size={15}/>{t('dash.nextStep')}</div><h2 className="text-2xl md:text-3xl font-semibold tracking-[-0.03em] max-w-3xl">{project.nextAction}</h2><div className="text-white/70 text-sm mt-3">{project.course} · {project.title}</div><div className="mt-5 h-1.5 rounded-full bg-white/15 overflow-hidden max-w-xl"><div className="h-full bg-white rounded-full" style={{ width: `${project.progress}%` }}/></div></div><Button asChild variant="secondary" className="bg-white text-[var(--brand-2)] hover:bg-white/90"><Link to={`/app/project/${project.id}`}>{t('dash.openWorkspace')} <ArrowRight size={16} className="directional-icon"/></Link></Button></div></section>;
}
...
function ProjectRow({ project }: { project: ProjectDNA }) {
  return <Link to={`/app/project/${project.id}`} className="focus-ring panel-flat rounded-2xl p-4 md:p-5 block hover:shadow-[var(--shadow)] transition-shadow"><div className="flex gap-4 items-center"><div className="h-11 w-11 rounded-xl tone-tile shrink-0"><FolderKanban size={18}/></div><div className="min-w-0 flex-1"><div className="flex items-center gap-2 flex-wrap"><span className="text-xs muted">{project.course}</span><StatusPill status={project.status}/></div><h3 className="text-sm md:text-base font-semibold truncate mt-1.5">{project.title}</h3><div className="mt-3 flex items-center gap-3"><div className="h-1.5 rounded-full bg-[var(--panel-2)] overflow-hidden flex-1"><div className="h-full brand-bg rounded-full" style={{ width: `${project.progress}%` }}/></div><span className="text-[11px] muted mono-number">{project.progress}%</span></div></div><ArrowRight size={18} className="muted shrink-0 directional-icon"/></div></Link>;
}
