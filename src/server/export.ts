...
export interface ExportBranding { institutionName?: string; footer?: string }
...
export function projectExportHtml(project:ProjectDNA,artifacts:WorkspaceArtifact[]=[],branding:ExportBranding={}){
  ...
  const manuscript=sections.length?`<h2>المشروع الأكاديمي</h2>${sections.map(section=>`<section><h2>${e(section.title)}</h2><div class="section-content">${e(section.content).replace(/\n/g,'<br>')}</div></section>`).join('')}`:'';
  return `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><style>@page{size:A4;margin:18mm}*{box-sizing:border-box}body{font-family:Arial,"Noto Sans Arabic",sans-serif;color:#17211e;line-height:1.8;font-size:11pt}h1{font-size:25pt;margin:0 0 6mm}h2{font-size:15pt;margin:8mm 0 3mm;border-bottom:1px solid #d9dfdc;padding-bottom:2mm}.meta{display:grid;grid-template-columns:1fr 1fr;gap:2mm 8mm;background:#f6f7f5;padding:4mm;border-radius:3mm}.muted{color:#66736e}.section-content{white-space:normal;text-align:justify}li{margin-bottom:1.5mm}.footer{margin-top:12mm;border-top:1px solid #d9dfdc;padding-top:3mm;font-size:8.5pt;color:#66736e}</style></head><body>${branding.institutionName?`<div class="muted">${e(branding.institutionName)}</div>`:''}<h1>${e(project.title)}</h1><div class="muted">AcademicOS — مشروع + فهم + إثبات</div><div class="meta"><div><strong>المقرر</strong><br>${e(project.course)}</div><div><strong>النوع</strong><br>${e(project.projectType)}</div><div><strong>الحالة</strong><br>${e(project.status)} — ${e(project.progress)}%</div><div><strong>الموعد</strong><br>${e(project.deadlines.final||'Needs confirmation')}</div><div><strong>AI Policy</strong><br>Level ${e(project.aiPolicy.level)} — ${e(project.aiPolicy.summary)}</div><div><strong>المجال</strong><br>${e(project.academicDomain)}</div></div>${manuscript}<h2>المخرجات</h2>${list(project.deliverables.map(d=>`<strong>${e(d.title)}</strong> — ${e(d.format)} — ${e(d.status)}`))}<h2>المتطلبات</h2>${list(project.requirements.map(r=>`<strong>${e(r.label)}</strong>: ${e(r.value)} <span class="muted">(${e(r.confidence)})</span>`))}<h2>الـRubric</h2>${list(project.rubric.length?project.rubric.map(r=>`<strong>${e(r.title)}</strong> (${e(r.weighting)}%) — ${e(r.readiness||'not evidenced')}`):['لا يوجد Rubric مورّد أو مكتشف.'])}<h2>المخاطر والتأكيدات</h2>${list(project.riskFlags.length?project.riskFlags.map(e):['لا توجد مخاطر مسجلة حاليًا.'])}<div class="footer">تم إنشاء هذا السجل من AcademicOS بتاريخ ${e(new Date().toISOString())}. يجب التحقق من المصادر والبيانات وسياسة المقرر قبل التسليم.${branding.footer?` ${e(branding.footer)}`:''}</div></body></html>`;
}

function markdown(project: ProjectDNA, artifacts: WorkspaceArtifact[] = [], branding:ExportBranding={}) {
  const sections=academicSections(artifacts);
  const lines = [
    ...(branding.institutionName?[`**Institution:** ${branding.institutionName}`,'']:[]),`# ${project.title}`,'',`- **Course:** ${project.course}`,`- **Project type:** ${project.projectType}`,`- **Status:** ${project.status}`,`- **Progress:** ${project.progress}%`,
    ...(sections.length?['','## Academic project',...sections.flatMap(section=>['',`### ${section.title}`,'',section.content])]:[]),
    `- **Deadline:** ${project.deadlines.final || 'Needs confirmation'}`,`- **AI policy:** Level ${project.aiPolicy.level} — ${project.aiPolicy.summary}`,'','## Deliverables',
    ...project.deliverables.map(d => `- [${d.status === 'completed' || d.status === 'ready' ? 'x' : ' '}] ${d.title} — ${d.format}`),'','## Requirements',
    ...project.requirements.map(r => `- **${r.label}:** ${r.value} (${r.confidence})`),'','## Tasks',...project.tasks.map(t => `- [${t.status === 'completed' ? 'x' : ' '}] ${t.title} — ${t.status}`),'','## Rubric',
    ...(project.rubric.length ? project.rubric.map(r => `- **${r.title}** (${r.weighting}%): ${r.description || 'No description'} — ${r.readiness || 'not_evidenced'}`) : ['- No rubric was supplied or detected.']),'','## Risks / confirmations',
    ...(project.riskFlags.length ? project.riskFlags.map(r => `- ${r}`) : ['- None currently recorded.']),'','## Adaptive workspace artifacts',
    ...(artifacts.length ? artifacts.map(a => `- **${a.title}** — ${a.module} / ${a.status}${a.deliverableId ? ` → deliverable ${a.deliverableId}` : ''}${a.rubricIds?.length ? ` → rubric ${a.rubricIds.join(', ')}` : ''}${a.isCanonical ? ' — canonical work item' : ''}`) : ['- No saved workspace artifacts.']),'','---',
    `Exported from AcademicOS at ${new Date().toISOString()}. This export is a project record, not a grade prediction.`,
  ]; return lines.join('\n');
}
...
function projectRows(project:ProjectDNA,branding:ExportBranding={}){return [
  ['Field','Value'],...(branding.institutionName?[['Institution',branding.institutionName]]:[]),['Title',project.title],['Course',project.course],['Project type',project.projectType],['Academic domain',project.academicDomain],['Status',project.status],['Progress',`${project.progress}%`],['Deadline',project.deadlines.final||'Needs confirmation'],['AI policy',`Level ${project.aiPolicy.level} — ${project.aiPolicy.summary}`],
  ...project.deliverables.map(d=>[`Deliverable: ${d.title}`,`${d.format} — ${d.status}`]),...project.tasks.map(t=>[`Task: ${t.title}`,t.status]),...project.rubric.map(r=>[`Rubric: ${r.title}`,`${r.weighting}% — ${r.readiness||'not evidenced'}`]),
];}

function docx(project:ProjectDNA,artifacts:WorkspaceArtifact[],branding:ExportBranding={}){
  const sections=academicSections(artifacts);
  const paras=[...(branding.institutionName?[branding.institutionName]:[]),project.title,`Course: ${project.course}`,`Project type: ${project.projectType}`,`Deadline: ${project.deadlines.final||'Needs confirmation'}`,...sections.flatMap(section=>[section.title,...section.content.split(/\n+/).filter(Boolean)]),'Rubric',...project.rubric.map(r=>`${r.title} (${r.weighting}%): ${r.description||''} — ${r.readiness||'not evidenced'}`),'AI disclosure',project.aiPolicy.summary];
  const sectionTitles=new Set(sections.map(section=>section.title));
  const p=paras.map((t,i)=>`<w:p${i===0?' w:rsidR="00000000"':''}><w:pPr><w:bidi/><w:jc w:val="${i===0?'center':'both'}"/>${sectionTitles.has(t)?'<w:spacing w:before="300" w:after="120"/>':''}</w:pPr><w:r><w:rPr>${i===0||sectionTitles.has(t)?'<w:b/>':''}<w:rtl/></w:rPr><w:t xml:space="preserve">${xml(t)}</w:t></w:r></w:p>`).join('');
  return zipStore([
...
function pptx(project:ProjectDNA,branding:ExportBranding={}){
  const slides=[
    ...
    ['Rubric readiness',project.rubric.map(r=>`${r.title}: ${r.readiness||'not evidenced'}`).join('\n')||'No rubric supplied'],
    ['Next action',project.nextAction||'Review the project plan and requirements.'],
  ];
...
function xlsx(project:ProjectDNA,branding:ExportBranding={}){
  ...
  return zipStore([
    ...
    {name:'xl/worksheets/sheet1.xml',data:b(`<?xml version="1.0" encoding="UTF-8"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetViews><sheetView rightToLeft="1" workbookViewId="0"/></sheetViews><sheetData>${sheet}</sheetData></worksheet>`)},
  ]);
}
...
export function exportProject(project: ProjectDNA, format: string, artifacts: WorkspaceArtifact[] = [], branding:ExportBranding={}) {
  ...
  if(format==='zip'){
    ...
    const readme=Buffer.from('AcademicOS Submission Package\n\nThis package contains editable Office exports plus canonical JSON/Markdown/CSV representations. Verify filenames and course submission requirements before final submission. AI disclosure requirements remain governed by the project policy in submission-manifest.json.\n','utf8');
...
export function exportCitations(evidence: Array<{id:string;type:string;title:string;detail:string;sourceUrl?:string;verification:string}>, format='ris') {
...
export function exportLearningEvidenceReport(project:ProjectDNA,evidence:Array<{source:string;summary:string;evidence:Array<{label:string;value:string}>;createdAt:string}>,format='md'){
  const payload={project:{id:project.id,title:project.title,course:project.course},generatedAt:new Date().toISOString(),learningEvidence:evidence,notice:'Learning evidence documents observed demonstrations and does not predict a final grade.'};
  ...
  const lines=[`# Learning Evidence — ${project.title}`,'',`Course: ${project.course}`,'','This report documents observed learning evidence. It is not an AI-detector score or a final-grade prediction.','',...evidence.flatMap((x,i)=>[`## Evidence ${i+1} — ${x.source}`,x.summary,`Created: ${x.createdAt}`,...x.evidence.map(e=>`- **${e.label}:** ${e.value}`),''])];
  return{data:Buffer.from(lines.join('\n'),'utf8'),contentType:'text/markdown; charset=utf-8',filename:'AcademicOS-learning-evidence.md'};
}

export function exportCourseArchive(course:{id:string;code:string;title:string;term?:string;outcomes:string[];aiPolicy:unknown;status:string;createdAt:string;updatedAt:string},assignments:Array<Record<string,unknown>>){
  const archive={schemaVersion:1,exportedAt:new Date().toISOString(),course,assignments};
  const md=[`# ${course.code} — ${course.title}`,course.term?`Term: ${course.term}`:'',`Status: ${course.status}`,'','## Outcomes',...course.outcomes.map(x=>`- ${x}`),'','## Assignments',...assignments.map((x:any)=>`- ${x.title||x.id} — ${x.status||'unknown'}`)].filter(Boolean).join('\n');
  const manifest={schemaVersion:1,courseId:course.id,files:['course.json','course.md','assignments.json'],records:{assignments:assignments.length},exportedAt:new Date().toISOString()};
  return{data:zipStore([{name:'course.json',data:Buffer.from(JSON.stringify(course,null,2),'utf8')},{name:'course.md',data:Buffer.from(md,'utf8')},{name:'assignments.json',data:Buffer.from(JSON.stringify(assignments,null,2),'utf8')},{name:'archive-manifest.json',data:Buffer.from(JSON.stringify(manifest,null,2),'utf8')}]),contentType:'application/zip',filename:`${course.code.replace(/[^A-Za-z0-9._-]+/g,'_')||'AcademicOS-course'}-archive.zip`};
}
