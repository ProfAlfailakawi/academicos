import { AppDialog, DialogFrame } from "../components/AppDialog";
...
export function PlatformHub(){
 ...
 {records.length?<div className="grid lg:grid-cols-2 gap-3">{records.map(r=><Card key={r.id}><CardContent><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="eyebrow">v{r.version} · {platformStatusLabel(r.status,locale)}</div><h3 className="font-semibold mt-1 truncate">{r.title}</h3><div className="text-[11px] muted mt-1">{formatDateTime(r.updatedAt,locale)}</div></div><ChevronRight size={16} className="muted directional-icon"/></div><div className="mt-4 rounded-xl bg-[var(--bg)] p-3 max-h-40 overflow-auto"><pre className="text-[11px] whitespace-pre-wrap break-words font-mono" dir="ltr">{pretty(r.data)}</pre></div><div className="mt-4 flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={()=>history(r)}><History size={13}/> {t('plat.versions')}</Button>{meta?.write&&!r.deletedAt&&<Button size="sm" variant="outline" onClick={()=>openEdit(r)}><Save size={13}/> {t('plat.edit')}</Button>}{meta?.write&&!r.deletedAt&&<Button size="sm" variant="outline" onClick={()=>remove(r)}><Trash2 size={13}/> {t('plat.softDelete')}</Button>}{meta?.write&&r.deletedAt&&<Button size="sm" variant="outline" onClick={()=>restore(r)}><RotateCcw size={13}/> {t('plat.restore')}</Button>}</div></CardContent></Card>)}</div>:<Card><CardContent className="py-12 text-center"><Boxes size={24} className="mx-auto muted"/><div className="font-semibold mt-3">{t('plat.noRecords')}</div><p className="body-copy mt-1">{t('plat.noRecordsHint')}</p></CardContent></Card>}
 </div></div>
 {editor&&<DialogFrame onClose={()=>setEditor(null)} labelledBy="plat-editor-title" overlayClassName="fixed inset-0 z-[80] flex items-end lg:items-center justify-center p-3" backdropClassName="absolute inset-0 bg-black/35" className="relative panel w-full max-w-2xl rounded-3xl p-5 max-h-[90vh] overflow-auto"><div className="flex justify-between gap-3"><div><div className="eyebrow">{editor.record?t("ui.editVersionedRecord"):t("ui.createRecord")}</div><h2 id="plat-editor-title" className="section-title mt-1">{meta?.label}</h2></div><button onClick={()=>setEditor(null)} className="focus-ring p-2 rounded-lg" aria-label={t("ui.close")}><X size={17}/></button></div><label className="block mt-5 text-xs font-semibold">{t('plat.fieldTitle')}<input value={editor.title} onChange={e=>setEditor({...editor,title:e.target.value})} className="field mt-2 w-full"/></label><label className="block mt-4 text-xs font-semibold">{t('plat.fieldStatus')}<select value={editor.status} onChange={e=>setEditor({...editor,status:e.target.value})} className="field mt-2 w-full">{meta?.statusValues.map(x=><option key={x} value={x}>{platformStatusLabel(x,locale)}</option>)}</select></label><label className="block mt-4 text-xs font-semibold">{t('plat.fieldData')}<textarea dir="ltr" value={editor.json} onChange={e=>setEditor({...editor,json:e.target.value})} rows={15} className="field mt-2 w-full font-mono text-xs resize-y"/></label><div className="text-[11px] muted mt-2">{t('plat.suggestedFields').replace('{fields}',meta?.suggestedFields.join(' · ')||'—')}</div><div className="mt-5 flex justify-end gap-2"><Button variant="outline" onClick={()=>setEditor(null)}>{t('plat.cancel')}</Button><Button disabled={busy||!editor.title.trim()} onClick={save}><Save size={14}/> {t('plat.save')}</Button></div></DialogFrame>}
 {versions&&<DialogFrame onClose={()=>setVersions(null)} labelledBy="plat-versions-title" overlayClassName="fixed inset-0 z-[85] flex items-end lg:items-center justify-center p-3" backdropClassName="absolute inset-0 bg-black/35" className="relative panel w-full max-w-2xl rounded-3xl p-5 max-h-[85vh] overflow-auto"><div className="flex justify-between"><div><div className="eyebrow">{t("ui.immutableHistory")}</div><h2 id="plat-versions-title" className="section-title mt-1">{t('plat.versionHistory')}</h2></div><button onClick={()=>setVersions(null)} className="focus-ring p-2" aria-label={t("ui.close")}><X size={17}/></button></div><div className="mt-5 space-y-3">{versions.map(v=><div key={v.id} className="rounded-xl bg-[var(--bg)] border hairline p-3"><div className="flex justify-between"><strong className="text-xs">{t("ui.version")} {v.version}</strong><span className="text-[11px] muted">{formatDateTime(v.createdAt,locale)}</span></div><div className="text-[11px] muted mt-1">{v.reason||t("ui.noReasonSupplied")}</div></div>)}</div></DialogFrame>}
