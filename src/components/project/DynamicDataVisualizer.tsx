  ...
  return <div className="space-y-6">
    ...
    <Card><CardContent className="p-6 md:p-8 space-y-6">
      ...
      {chartType === "flowchart" && <div className="space-y-5"><div><h3 className="font-bold text-sm">{t("visual.workflowTitle")}</h3><p className="text-xs text-muted-foreground mt-1">{t("visual.workflowDesc")}</p></div>
        {workflowSteps.length ? <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">{workflowSteps.map((step, idx) => <div key={`${step.title}-${idx}`} className="rounded-2xl border hairline bg-[var(--bg)] p-4"><div className="flex items-center justify-between"><span className="h-7 w-7 rounded-lg bg-info text-white text-xs font-bold grid place-items-center">{idx + 1}</span><span className="text-[11px] font-mono text-info">{step.time}</span></div><div className="font-bold text-xs mt-3">{step.title}</div><p className="text-[11px] text-muted-foreground leading-relaxed mt-1">{step.desc}</p></div>)}</div> : <div className="rounded-xl border hairline p-6 text-xs text-muted-foreground">{t("visual.noWorkflow")}</div>}
